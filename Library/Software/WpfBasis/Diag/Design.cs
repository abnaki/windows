...
namespace Abnaki.Windows.Software.Wpf.Diag
{
    ...
    public class Design
    {
        ...
        static void SubDebugAncestry(DependencyObject x)
        {
            ...
            {
                Debug.Write(AboutFrameworkElement((FrameworkElement)x));
            }
            Debug.WriteLine("");
        }

        static string AboutFrameworkElement(FrameworkElement x)
        {
            return ", DataContext=" + x.DataContext + ", VerticalAlignment=" + x.VerticalAlignment + ", Height=" + x.Height;
        }
